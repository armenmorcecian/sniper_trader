...
export class NewsService {
   ...
   * Fetches crypto news, filters by keywords, returns raw headlines.
  ...
  async validateWithPremiumNews(keywords: string[]): Promise<NewsResult> {
    ...
    try {
      ...
      // NewsAPI failed — return RSS-only results
      return {
        ...rssResult,
        premiumValidated: false,
      };
  ...
  private async parseFeed(
    ...
    return (feed.items || [])
      .filter((item) => {
        ...
        // Filter by keyword match in title or content
