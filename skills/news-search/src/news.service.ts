...
import { NewsItem, NewsResult, WatchNewsResult, SeenHeadlinesState, ScoredNewsItem } from "./types";
import { scoreSentiment } from "./sentiment";

// Combined feeds from both trading skills (12 unique feeds)
const RSS_FEEDS = [
  // Macro / General
...
export class NewsService {
   ...
   * Fetches news, filters by keywords, returns raw headlines.
   * The LLM agent reads headlines and decides sentiment itself.
   */
  async fetchRelevantNews(
    ...
    const scored: ScoredNewsItem[] = deduped.slice(0, 30).map((item) => ({
      ...item,
      sentiment: scoreSentiment(item.title, item.summary),
    }));
  ...
  async validateWithPremiumNews(keywords: string[]): Promise<NewsResult> {
    ...
    try {
      ...
      const scored: ScoredNewsItem[] = merged.slice(0, 40).map((item) => ({
        ...item,
        sentiment: scoreSentiment(item.title, item.summary),
      }));
   ...
   */
  async watchNews(): Promise<WatchNewsResult> {
    ...
    const scored: ScoredNewsItem[] = newItems.slice(0, 50).map((item) => ({
      ...item,
      sentiment: scoreSentiment(item.title, item.summary),
    }));
  ...
  /** Returns current daily NewsAPI usage count */
  ...
  private loadSeenHeadlines(): SeenHeadlinesState {
  ...
  private saveSeenHeadlines(state: SeenHeadlinesState): void {
