  ...
  async init(): Promise<void> {
    try {
      const timeout = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("getPortfolioValue timeout")), 10_000),
      );
      const vitals = await Promise.race([this.service.getPortfolioValue(), timeout]);
      if (vitals.positions) {
    ...
    // Run orphan redemption with a 12s timeout — Polygon RPC can hang; never block init
    await Promise.race([
      this.redeemOrphanedWinners(),
      new Promise<void>((resolve) => setTimeout(() => {
        console.warn(`${LOG_PREFIX} redeemOrphanedWinners() timed out after 12s — continuing`);
        resolve();
      }, 12_000)),
    ]);
  }
    ...
    try {
      ...
      if (fillPrice > this.config.maxWinningPrice) {
        ...
        if (sellBackSucceeded) {
          // Net position is zero after sell-back — release dedup lock so we can retry
